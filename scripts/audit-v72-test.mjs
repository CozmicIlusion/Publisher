...
check("content.article-count", slugs.length === 17, `${slugs.length} article slugs`);
...
check("rss.item-count", (rss.match(/<item>/g) || []).length === 17, `${(rss.match(/<item>/g) || []).length} RSS items`);

const sitemap = readFileSync(resolve(dist, "sitemap.xml"), "utf8");
const sitemapUrls = (sitemap.match(/<loc>/g) || []).length;
check("sitemap.url-count", sitemapUrls === 31, `${sitemapUrls} sitemap URLs`);
